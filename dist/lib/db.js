...
export async function checkJobExists(orgName, orderNumber, checkImages = true) {
    try {
        // Get organization
        ...
        const [existingOrder] = await db
            .select()
            .from(schema.orders)
            .where(and(eq(schema.orders.order_number, orderNumber), eq(schema.orders.organization_id, org.id)));
        if (!existingOrder) {
            return { exists: false, hasImages: false };
        }
        ...
        console.error(`Error checking if job exists for order ${orderNumber}:`, error);
        return { exists: false, hasImages: false };
    }
}
// Function to validate and sanitize scraped job data
function validateScrapedJob(jobData) {
    const errors = [];
    ...
    if (!jobData.customer?.name || !jobData.customer.name.trim()) {
        ...
        if (sanitizedData.customer.name.length > 100) {
            console.warn(`Customer name too long (${sanitizedData.customer.name.length} chars), truncating`);
    ...
    if (!jobData.order?.title || !jobData.order.title.trim()) {
        ...
        if (sanitizedData.order.title.length > 255) {
            console.warn(`Order title too long (${sanitizedData.order.title.length} chars), truncating`);
...
async function ensureCustomer(orgId, name) {
    try {
        ...
        const [existingCustomer] = await db
            ...
            .where(and(eq(schema.customers.organization_id, orgId), eq(schema.customers.name, name)));
        return existingCustomer;
    }
    catch (error) {
        const errorInfo = handleDatabaseError(error, `ensureCustomer for "${name}"`);
        // For certain errors, try to recover by finding existing customer
        if (errorInfo.category === "validation" ||
            errorInfo.category === "constraint") {
            console.warn(`Attempting to find existing customer after error: ${errorInfo.message}`);
            try {
                const [existingCustomer] = await db
                    .select()
                    .from(schema.customers)
                    .where(and(eq(schema.customers.organization_id, orgId), eq(schema.customers.name, name)));
                if (existingCustomer) {
                    console.log(`Successfully found existing customer: ${existingCustomer.name}`);
                    return existingCustomer;
                }
            }
            catch (recoveryError) {
                console.error("Failed to recover by finding existing customer:", recoveryError);
            }
        }
        throw error;
    }
...
export async function ensureTags(orgId) {
...
export async function saveScrapedJob(orgName, jobData) {
    try {
        console.log(`Processing job ${jobData.jobNumber || "unknown"} for ${orgName}`);
        ...
        try {
            ...
            if (!customer) {
                throw new Error(`Failed to create or find customer: ${sanitizedJobData.customer.name}`);
            }
        }
        catch (error) {
            console.error(`Customer creation failed for job ${sanitizedJobData.jobNumber}:`, error);
            throw error;
        }
        ...
        console.log("Scraped tags for job", sanitizedJobData.order.order_number, ":", sanitizedJobData.tags);
        ...
        try {
            [existingOrder] = await db
                .select()
                .from(schema.orders)
                .where(eq(schema.orders.order_number, sanitizedJobData.order.order_number));
        }
        catch (error) {
            console.warn(`Failed to check for existing order ${sanitizedJobData.order.order_number}:`, error);
            // Continue with insert operation
        }
        let order;
        if (existingOrder) {
            console.log(`Updating existing order: ${sanitizedJobData.order.order_number}`);
            ...
            try {
                ...
                const errorInfo = handleDatabaseError(error, `update order ${sanitizedJobData.order.order_number}`);
            ...
            try {
                ...
                const errorInfo = handleDatabaseError(error, `create order ${sanitizedJobData.order.order_number}`);
                throw new Error(`Failed to create order: ${errorInfo.message}`);
            }
        }
        // 6. Create order tags with quantities
        const validTags = sanitizedJobData.tags.filter((tag) => tag.code.toUpperCase() in TAG_MAP && tagMap.has(tag.code.toUpperCase()));
        if (validTags.length > 0) {
            ...
            try {
                ...
                console.log(`Created ${orderTagValues.length} order tags for order ${sanitizedJobData.order.order_number}`);
            }
            catch (error) {
                console.warn(`Failed to create order tags for ${sanitizedJobData.order.order_number}:`, error);
                // Don't fail the entire operation for tag creation failures
            }
        }
        else {
            console.warn(`No valid tags found for order ${sanitizedJobData.order.order_number}`);
        }
        console.log(`✅ Successfully saved job ${sanitizedJobData.jobNumber} (order: ${sanitizedJobData.order.order_number})`);
        return order;
    }
    catch (error) {
        console.error(`❌ Error saving scraped job ${jobData.jobNumber || "unknown"}:`, error);
        throw error;
    }
}
// Function to save multiple scraped jobs
export async function saveScrapedJobs(orgName, jobs) {
    ...
    for (const job of jobs) {
        processed++;
        try {
            console.log(`Processing job ${processed}/${jobs.length}: ${job.jobNumber || "unknown"}`);
            ...
            if (processed % 10 === 0) {
                ...
                console.log(`Progress: ${processed}/${jobs.length} jobs (${rate.toFixed(2)} jobs/sec)`);
            }
        }
        catch (error) {
            ...
            if (error.message?.includes("validation failed")) {
                ...
                const errorInfo = handleDatabaseError(error, `save job ${job.jobNumber}`);
                category = errorInfo.category;
            }
            errorStats[category]++;
            errors.push({
    ...
    console.log(`📈 Success rate: ${successRate.toFixed(1)}% (${results.length}/${jobs.length})`);
    ...
    if (errors.length > 0) {
        ...
        sampleErrors.forEach((errItem, index) => {
            console.log(`   ${index + 1}. Job ${errItem.job.jobNumber}: ${errItem.error.message}`);
        });
        if (errors.length > 3) {

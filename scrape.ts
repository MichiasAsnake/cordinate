...
async function extractListData(row: any): Promise<{
  ...
  try {
    ...
    console.log(
      `📝 Extracted ${jobDescriptions.length} job descriptions for job ${jobNumber}`
    );
...
> {
  ...
  try {
    ...
    console.log(
      `🔍 Found ${imageContainers.length} image containers for job ${jobNumber}`
    );
    ...
    for (let i = 0; i < imageContainers.length; i++) {
      ...
      console.log(
        `🔍 Processing container ${i + 1}/${imageContainers.length}...`
      );
      ...
      try {
        ...
        if (!assetTag) {
          console.log(
            `   ⚠️ No asset tag found for container ${i + 1}, skipping`
          );
          continue;
        }
        ...
        if (!img || !link) {
          console.log(
            `   📭 Container ${assetTag} is empty (no images), skipping`
          );
          continue;
        }
        ...
        console.log(
          `   🖼️  Thumbnail URL: ${thumbnailUrl?.substring(0, 80)}...`
        );
        ...
        if (!thumbnailUrl || !highResUrl) {
          console.log(
            `   ⚠️ Missing URLs for ${assetTag} - thumbnail: ${!!thumbnailUrl}, high-res: ${!!highResUrl}`
          );
          continue;
        }
        ...
        if (!isThumbnailValid) {
          console.log(
            `   ⚠️ Invalid thumbnail URL for ${assetTag}: ${thumbnailUrl.substring(
              0,
              80
            )}...`
          );
        ...
        if (!isHighResValid) {
          console.log(
            `   ⚠️ Invalid high-res URL for ${assetTag}: ${highResUrl.substring(
              0,
              80
            )}...`
          );
    ...
    console.log(
      `✅ Successfully processed ${processedImages.length} images for job ${jobNumber}`
    );
    ...
    console.log(
      `   ✅ Valid thumbnails: ${validThumbnails.length}/${processedImages.length}`
    );
    console.log(
      `   ✅ Valid high-res: ${validHighRes.length}/${processedImages.length}`
    );
...
async function extractJobData(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `❌ Navigation failed for job ${jobNumber}:`,
        navigationError
      );
      return {
        ...listData,
    ...
    try {
      ...
      console.error(
        `❌ Image extraction failed for job ${jobNumber}:`,
        imageError
      );
      images = [];
    }
    ...
    try {
      ...
      console.log(
        `📍 Back to main job list, now navigating to page ${currentPageNumber}`
      );
      ...
      console.error(
        `❌ Failed to return to job list page ${currentPageNumber} after job ${jobNumber}:`,
        backError
      );
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        try {
          console.log(
            `🔄 Attempting to recover to page ${currentPageNumber}...`
          );
      ...
      try {
        ...
        if (i > 1) {
          ...
          try {
            ...
            console.error(
              `❌ Failed to click pagination for page ${i}:`,
              clickError
            );
            continue;
          }
          ...
          try {
            ...
            if (currentPageElement) {
              ...
              if (currentPageNum !== i.toString()) {
                console.warn(
                  `⚠️ Expected page ${i} but on page ${currentPageNum}`
                );
              }
            }
        ...
        console.log(
          `📊 Total progress: ${allJobs.length} jobs from ${i}/${totalPages} pages`
        );
      } catch (error) {
        ...
        try {
          ...
          console.error(
            `❌ Failed to recover from page ${i} error:`,
            recoveryError
          );
        }
